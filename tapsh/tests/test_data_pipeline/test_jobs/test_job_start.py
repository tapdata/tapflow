from tapsh.tests.test_data_pipeline.test_jobs import BaseJobTest
from unittest.mock import patch, Mock
from tapsh.lib.data_pipeline.job import Job

class TestJobStart(BaseJobTest):
    @patch('tapsh.lib.data_pipeline.job.time')
    @patch('tapsh.lib.data_pipeline.job.logger.warn')
    @patch('tapsh.lib.data_pipeline.job.logger.info')
    @patch('tapsh.lib.data_pipeline.job.req.put')
    @patch('tapsh.lib.data_pipeline.job.Job.save')
    @patch('tapsh.lib.data_pipeline.job.Job.status')
    @patch('tapsh.lib.data_pipeline.job.req.get')
    @patch('tapsh.lib.op_object.get_obj')
    @patch('tapsh.lib.data_pipeline.job.client_cache', new_callable=dict)
    def test_start_when_status_is_edit(self, mock_client_cache, mock_get_obj, mock_req_get, mock_status, 
        ...
        # 模拟start_task的响应
        with patch('tapsh.lib.backend_apis.task.TaskApi.start_task') as mock_start_task:
    ...
    @patch('tapsh.lib.data_pipeline.job.logger.fwarn')
    @patch('tapsh.lib.data_pipeline.job.req.put')
    @patch('tapsh.lib.data_pipeline.job.Job.status')
    @patch('tapsh.lib.data_pipeline.job.req.get')
    @patch('tapsh.lib.op_object.get_obj')
